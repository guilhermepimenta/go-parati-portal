...
const BusinessDetail: React.FC<BusinessDetailProps> = ({ business, userLocation, onBack }) => {
  ...
  const handleOpenExternalMaps = () => {
    analytics.trackDirections(business.id);
  ...
  useEffect(() => {
    ...
    fetchReviews();
    analytics.trackPageView('business_detail', { id: business.id, name: business.name });
  ...
  }, [business, images, contactPhone]);

  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      <Helmet>
        <title>{`${business.name} - Paraty, RJ | Go Parati`}</title>
        <meta name="description" content={business.description} />
        <meta property="og:title" content={`${business.name} - Go Parati`} />

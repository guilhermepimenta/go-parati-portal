...
import { TOTEM_DATA } from '../constants';
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <p className="text-4xl font-black tracking-tighter">{formatDistance(calculateDistance(userLocation.lat, userLocation.lng, nearestTotem.location.lat, nearestTotem.location.lng))}</p>
                    </div>
              ...
              <div className="space-y-3">
                {sortedTotems.map(totem => {
                  ...
                  return (
                    <div
                      key={totem.id}
                      className="w-full flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-white hover:border-sky-200 hover:shadow-lg hover:shadow-sky-500/5 transition-all group"
                    >
                      <div className="flex items-center gap-4 cursor-pointer" onClick={() => handleFocusTotem(totem)}>
                        <div className={`p-2.5 rounded-xl transition-colors duration-300 ${isOnlineStatus ? 'bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100' : 'bg-rose-50 text-rose-500 group-hover:bg-rose-100'}`}>
                          {isOnlineStatus ? <CheckCircle2 className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
                        </div>
                        <div className="text-left">
                          <p className="text-sm font-bold text-slate-900 group-hover:text-sky-600 transition-colors flex items-center gap-2">
                            {totem.name}
                            {!isOnlineStatus && <span className="text-[9px] bg-rose-100 text-rose-600 px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter">Offline</span>}
                          </p>
                          <p className="text-[10px] text-slate-400 font-medium">{totem.location.address}</p>
                        </div>
                      </div>

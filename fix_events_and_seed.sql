...
-- 1. Enable the pgcrypto extension if it's not already enabled (needed for gen_random_uuid())
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- 2. Fix the 'events' table to automatically generate IDs
ALTER TABLE public.events 
ALTER COLUMN id SET DEFAULT gen_random_uuid();

-- 3. Insert the Featured Event (now that ID will be auto-generated)
